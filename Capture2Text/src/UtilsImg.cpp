...
QImage UtilsImg::takeScreenshot(const QRect &rect)
{
    ...
    QPixmap capturePixmap = screen->grabWindow(0, qFloor(rect.x()/screen->devicePixelRatio()), qFloor(rect.y()/screen->devicePixelRatio())
                                               , qFloor(rect.width()/screen->devicePixelRatio()), qFloor(rect.height()/screen->devicePixelRatio()));
